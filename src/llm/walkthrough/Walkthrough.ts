...
import { walkthrough10_Compare } from "./Walkthrough10_Compare";
    ...
    return {
        ...
        phaseList: [{
            ...
            phases: [
                ...
                { id: Phase.Intro_Compare, title: 'DeepSeek 对比' },
            ],
        }, {
            groupId: PhaseGroup.Detailed_Input,
            title: '详细讲解',
            phases: [
                { id: Phase.Input_Detail_Embedding, title: '词嵌入' },
...
export function walkthroughDetailed(args: IWalkthroughArgs) {
    ...
    case Phase.Input_First: {
        let t0 = c_str('', 0);
        let c = commentary`这些向量现在依次经过模型的各个阶段，通过一系列 Transformer 模块。${t0}`;
        let t1 = atEvent(t0);
    ...
    case Phase.Input_Detail_Tables: {
        ...
        drawTextOnModel(render, '词嵌入矩阵', new Vec3(tokEmbed.x - layout.margin, tokEmbed.y + tokEmbed.dy / 4, 0), {
        ...
        drawTextOnModel(render, '位置嵌入矩阵', new Vec3(posEmbed.x + posEmbed.dx + layout.margin, tokEmbed.y + tokEmbed.dy / 4, 0), {
    ...
    case Phase.Input_Detail_TokEmbed: {
        let tStr = c_str('t', 1);
        let c = commentary`让我们从最上层开始。为了计算每个时刻 ${tStr} 的向量，我们需要做以下几步：`;
        ...
        let embedMtx = c_str('词嵌入矩阵');
        let tokCol = c_str('j');
        commentaryPara(c)`\n\n1. 从 ${embedMtx} 中，选出第 ${tokCol} 列。`;
