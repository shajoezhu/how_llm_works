...
自注意力层也许是 Transformer 与 GPT 的核心所在。在这一阶段，我们输入词嵌入矩阵的各列开始相互“交流”。在此之前以及所有其他阶段中，这些列都可以被看作相互独立的。

自注意力层由多个注意力头组成，目前我们先聚焦于其中一个。`;
    breakAfter();
    ...
    breakAfter();
    commentary(wt)`
第一步是从${c_blockRef('归一化后的输入词嵌入矩阵', block0.ln1.lnResid)}的${c_dimRef('T', DimStyle.T)}列中，为每一列生成三个向量。
这三个向量分别是 Q、K 和 V 向量：
...
为了生成其中某一个向量，我们执行一次矩阵-向量乘法并加上偏置。每个输出单元都是输入向量的某种线性组合。例如，对于${c_blockRef('Q 向量', head2.qBlock)}，这是通过${c_blockRef('Q 权重矩阵', head2.qWeightBlock)}的一行与${c_blockRef('输入矩阵', block0.ln1.lnResid)}的一列做点积来完成的。`;
    breakAfter();
    ...
    breakAfter();
    commentary(wt)`
点积运算（我们会频繁遇到）相当简单：我们将第一个向量中的每个元素与第二个向量中对应的元素配对，将配对相乘，然后将结果相加。`;
    breakAfter();
...
这是一种通用且简单的方法，可确保每个输出元素都能受到输入向量中所有元素的影响（该影响由权重决定）。因此它在神经网络中频繁出现。
...
我们对 Q、K、V 向量中的每个输出单元重复这一操作：`;
    ...
    breakAfter();
    commentary(wt)`
我们要如何使用这些 Q（查询）、K（键）和 V（值）向量呢？这个命名给了我们提示：“键”和“值”让人联想到软件中的字典，键映射到值。而“查询”则是我们用来查找值的东西。
...
在自注意力中，我们不是返回单个条目，而是返回这些条目的某种加权组合。为确定权重，我们计算一个 Q 向量与每个 K 向量之间的点积。我们对该权重进行归一化，最后用它去乘相应的 V 向量，然后将它们全部相加。
...
为了更具体地说明，让我们来看第 6 列（${c_dimRef('t = 5', DimStyle.T)}），我们将以它作为查询来源：`;
    breakAfter();
...
// Q (query) vector is what information is relevant to us.
    commentary(wt)`
我们查找表中的 {K, V} 条目是之前的 6 列，而 Q 值就是当前时刻。

我们首先计算当前列（${c_dimRef('t = 5', DimStyle.T)}）的${c_blockRef('Q 向量', head2.qBlock)}与之前每一列的${c_blockRef('K 向量', head2.kBlock)}之间的点积。然后这些结果被存入${c_blockRef('注意力矩阵', head2.attnMtx)}中相应的行（${c_dimRef('t = 5', DimStyle.T)}）。`;
    breakAfter();
    ...
    breakAfter();
    commentary(wt)`
这些点积是衡量两个向量之间相似度的一种方式。如果它们非常相似，点积就会很大；如果它们差别很大，点积就会很小甚至为负。

只将查询与过去的键进行比对，这一思路使得自注意力具有_因果性_。也就是说，词元无法“看到未来”。

另一个要点是：在取完点积之后，我们会除以 sqrt(${c_dimRef('A', DimStyle.A)})，其中${c_dimRef('A', DimStyle.A)}是 Q/K/V 向量的长度。这种缩放是为了防止较大的值在下一步的归一化（softmax）中占据主导。

我们将大致略过 softmax 运算（稍后介绍）；简而言之，每一行都会被归一化，使其和为 1。`;
    breakAfter();
    ...
    breakAfter();
    commentary(wt)`
最后，我们可以为当前列（${c_dimRef('t = 5', DimStyle.T)}）生成输出向量。我们查看${c_blockRef('归一化后的自注意力矩阵', head2.attnMtxSm)}的（${c_dimRef('t = 5', DimStyle.T)}）行，并逐元素地将每个元素与对应列的${c_blockRef('V 向量', head2.vBlock)}相乘。`;
    breakAfter();
    ...
    breakAfter();
    commentary(wt)`
然后我们可以将它们相加，得到输出向量。因此，输出向量将由得分较高的列所对应的 V 向量主导。

现在我们已经了解了整个过程，接下来让它对全部列运行一遍。`;
    ...
    breakAfter();
    commentary(wt)`
这就是自注意力层中的一个注意力头所经历的过程。因此，自注意力的主要目标是：每一列都试图从其他列中寻找相关信息并提取它们的值，而实现方式是将其_query_向量与那些其他列的_keys_进行比对。附加的限制是它只能查看过去的信息。
`;
