...
上一节中的 ${c_blockRef('_词嵌入_', state.layout.residual0)} 矩阵是我们第一个 Transformer 块的输入。

Transformer 块的第一步是对该矩阵应用 _层归一化_。这是一个
分别对矩阵每一列中的值进行归一化的操作。`;
    breakAfter();
    ...
    breakAfter();
    commentary(wt)`
归一化是深度神经网络训练中的一个重要步骤，它有助于提高
模型在训练过程中的稳定性。

我们可以分别看待每一列，因此现在先聚焦到第 4 列 (${c_dimRef('t = 3', DimStyle.T)})。`;
    ...
    breakAfter();
    commentary(wt)`
目标是使该列的平均值等于 0、标准差等于 1。为此，
我们计算该列的两个量（${c_blockRef('均值 (\u03bc)', ln.lnAgg1)} 与 ${c_blockRef('标准差 (\u03c3)', ln.lnAgg2)}），然后减去平均值并除以标准差。`;
    ...
    breakAfter();
    commentary(wt)`
这里我们用 E[x] 表示平均值，用 Var[x] 表示方差（列的长度等于 ${c_dimRef('C', DimStyle.C)}）。
方差就是标准差的平方。epsilon 项（ε = ${<>1&times;10<sup>-5</sup></>}）用于防止除以零。

由于这些值会应用到该列的所有元素，我们将它们计算并存储在聚合层中。

最后，得到归一化的值后，我们将列中每个元素乘以一个学习到的
${c_blockRef('权重 (\u03b3)', ln.lnSigma)}，然后加上一个 ${c_blockRef('偏置 (β)', ln.lnMu)} 值，从而得到我们的 ${c_blockRef('归一化值', ln.lnResid)}。`;
    ...
    breakAfter();
    commentary(wt)`
我们对 ${c_blockRef('输入词嵌入矩阵', layout.residual0)} 的每一列执行该归一化操作，结果为
${c_blockRef('归一化输入词嵌入', ln.lnResid)}，它已准备好传入自注意力层。
`;
