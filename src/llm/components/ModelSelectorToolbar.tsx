...
export const ModelSelectorToolbar: React.FC<{
    ...
    function makeButton(egIndex: number) {
        ...
        return <div key={egIndex} className={clsx('m-2 px-2 py-1 rounded shadow cursor-pointer hover:bg-blue-300', isActive ? 'bg-blue-200' : 'bg-white')} onClick={handleClick}>
            {example.name}
        </div>;
    }
    ...
    return <div className='absolute top-0 left-0 flex flex-col'>
        <div className='mt-2 ml-2 flex flex-row'>
            {makeButton(-1)}
            {progState.examples.map((ex, i) => ex.enabled ? makeButton(i) : null)}
        </div>
