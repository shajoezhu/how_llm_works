...
function constructNavLevels(model: IGptModelLayout) {
    ...
    function makeLayerNorm(name: string, ln: IGptLayerNormLayout): INavLevel {
        let agg: INavLevel = { name: "层归一化聚合", block: ln.lnAgg1 };
        let norm: INavLevel = { name: "归一化后", block: ln.lnResid };
        let bias: INavLevel = { name: "层归一化偏置", block: ln.lnSigma };
        let weight: INavLevel = { name: "层归一化权重", block: ln.lnMu };
        makeCol([agg, norm]);
    ...
    let transformers: INavLevel[] = model.blocks.map((block, i) => {
        ...
        function makeHead(head: IGptModelLayout['blocks'][0]['heads'][0], idx: number) {
            ...
            let qRow: INavLevel[] = [
                { name: "Q 偏置", block: head.qBiasBlock },
                { name: "Q 权重", block: head.qWeightBlock },
                { name: "Q 向量", block: head.qBlock },
            ];
            let kRow: INavLevel[] = [
                { name: "K 偏置", block: head.kBiasBlock },
                { name: "K 权重", block: head.kWeightBlock },
                { name: "K 向量", block: head.kBlock },
            ];

            let vRow: INavLevel[] = [
                { name: "V 偏置", block: head.vBiasBlock },
                { name: "V 权重", block: head.vWeightBlock },
                { name: "V 向量", block: head.vBlock },
            ];
            ...
            let attnSm: INavLevel = { name: "注意力 Softmax", block: head.attnMtxSm };
            let attnAgg1: INavLevel = { name: "注意力聚合 1", block: head.attnMtxAgg1 };
            let attnAgg2: INavLevel = { name: "注意力聚合 2", block: head.attnMtxAgg2 };
            let attn: INavLevel = { name: "注意力矩阵", block: head.attnMtx };
            let attnOut: INavLevel = { name: "注意力输出", block: head.vOutBlock };
        ...
        let projection: INavLevel = {
            name: "投影",
            children: [
                { name: "投影 偏置", block: block.projBias },
                { name: "投影 权重", block: block.projWeight },
                { name: "投影 输出", block: block.attnOut },
            ],
        };
        ...
        let fc1: INavLevel = {
            name: "MLP 全连接", // and activation
            children: [
                { name: "全连接 偏置", block: block.mlpFcBias },
                { name: "全连接 权重", block: block.mlpFcWeight },
                { name: "全连接 输出", block: block.mlpFc },
            ],
        };
        let act: INavLevel = {
            name: "MLP 激活",
        ...
        let fc2: INavLevel = {
            name: "MLP 投影",
            children: [
                { name: "全连接 偏置", block: block.mlpProjBias },
                { name: "全连接 权重", block: block.mlpProjWeight },
                { name: "全连接 输出", block: block.mlpResult },
            ]
        };
